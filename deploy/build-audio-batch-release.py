#!/usr/bin/env python3
"""Build a pinned thirteen-resource batch upload upgrade; inspect remains default."""
import argparse
import hashlib
import json
...
BASELINES = ('7ede1d73629da4aceebe8e75bb9fa5c3a3d0a858',
             ...
             '881de5c2face4ab3b9ecb113abc139fd30e8f155')
FILES = ('netlify/functions/video-upload-auth.js', 'netlify/functions/video-play-auth.js',
         ...
         'vod-upload.js', 'media-batch.js', 'media-batch-save.js', 'media-batch.css', 'media-batch-ui.js',
         'learning-center.js', 'watch.html', 'learning.html')
OPTIONAL_FILES = frozenset(('media-upload.js', 'media-player.js', 'media-player.css',
...
PRESERVED = ('server/index.js', 'server/package.json', 'netlify/functions/video-access.js',
             ...
             'media-upload.js', 'media-player.js', 'media-player.css', 'watch.html')
...
def manifest_resources(payload):
    if tuple(payload) != FILES:
        raise ValueError('Batch upload payload must match the fixed thirteen-file allowlist')
    resources = []
    for name in FILES:
        ...
        resources.append({'path': name, 'sha256': digest(payload[name]), 'size': len(payload[name]),
            ...
            'allow_missing': name in OPTIONAL_FILES and any(data is None for data in previous)})
    return resources
...
def build(commit, output):
    ...
    for name in PRESERVED:
        if git_bytes(commit, name) != git_bytes(BASELINES[-1], name):
            raise ValueError('Batch upload release changes a protected resource: ' + name)
    ...
    manifest = {'schema': 1, 'profile': 'audio-batch-v1', 'domain': 'kidneysphere.com',
    ...
    print(json.dumps({'file': str(output), 'sha256': digest(output.read_bytes()),
        ...
        'profile': 'audio-batch-v1', 'accepted_baselines': list(BASELINES),
        'first_step': 'python3 package.pyz --inspect (read-only)',
        'rollback_order': 'Roll back this batch update first; then use the original audio package for its own backup.'},
